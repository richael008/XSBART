...
Rcpp::List XBART_cpp(arma::mat y, arma::mat X, arma::mat Xtest, size_t num_trees, size_t num_sweeps, size_t max_depth, size_t n_min, size_t num_cutpoints, double alpha, double beta, double tau, double no_split_penality, size_t burnin = 1, size_t mtry = 0, size_t p_categorical = 0, double kap = 16, double s = 4, double tau_kap = 3, double tau_s = 0.5, bool verbose = false, bool sampling_tau = true, bool parallel = true, bool set_random_seed = false, size_t random_seed = 0, bool sample_weights_flag = true, double nthread = 0,size_t ctType=1,size_t RepeatT=5,double Sigmahat=1.0,size_t StepSize=10, bool MHB = true, bool NWB = false,bool FitB=true)
{
    ...
    // define model
    NormalModel *model = new NormalModel(kap, s, tau, alpha, beta, sampling_tau, tau_kap, tau_s,num_trees);
    ...
    std::unique_ptr<State> state(new NormalState(Xpointer, Xorder_std, N, p, num_trees, p_categorical, p_continuous, set_random_seed, random_seed, n_min, num_cutpoints, mtry, Xpointer, num_sweeps, sample_weights_flag, &y_std, Sigmahat, max_depth, y_mean, burnin, model->dim_residual, nthread, parallel,ctType,RepeatT,StepSize,MHB,NWB,FitB)); //last input is nthread, need update

    state->StartTime=begin;
    ...
    //COUT<<"Percent Matrix"<<std::endl;
    arma::mat PerMatrix_std(p, 1001, arma::fill::zeros);
    for (size_t i=0;i<p;i++)
    {
        for (size_t j=0;j<=1000;j++)
        {
            PerMatrix_std(i,j)=*(state->X_std + state->n_y * i + Xorder_std[i][j*(state->n_y-1)/1000]);
            
        }
        //COUT << i<<" "<<PerMatrix_std(i,0) << " "<<PerMatrix_std(i,50)<<" "<<PerMatrix_std(i,100) << " "<<std::endl;
      
    }
    
    ////////////////////////////////////////////////////////////////
    //COUT << "cppStartmcmc" << endl;

    //state->RecordTime(0,0);



    mcmc_loop(Xorder_std, verbose, sigma_draw_xinfo, *trees2, no_split_penality, state, model, x_struct,PerMatrix_std);
    //COUT << "EndStartmcmc" << endl;

    //state->RecordTime(1+state->num_sweeps*state->num_trees,0);
    model->predict_std(state->ctType,Xtestpointer, N_test, p, num_trees, num_sweeps, yhats_test_xinfo, *trees2);
    //state->RecordTime(1+state->num_sweeps*state->num_trees,0);
    // R Objects to Return
    ...
    //state->RecordTime(1+state->num_sweeps*state->num_trees,1);


    // ofstream Timefile;
    // Timefile.open ("log_Time.csv",ios::app);

    // for (size_t i = 0; i < num_sweeps*num_trees+2; i++)
    // {

    //     for (size_t j=0;j<8;j++)
    //     {
    //         Timefile<<state->TimeTable(i,j)<<",";
    //     } 
    //     Timefile<<std::endl; 
