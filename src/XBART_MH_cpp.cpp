...
Rcpp::List XBART_MH_cpp(arma::mat y, arma::mat X, arma::mat Xtest, size_t num_trees, size_t num_sweeps, size_t max_depth, size_t n_min, size_t num_cutpoints, double alpha, double beta, double tau, double no_split_penality, size_t burnin = 1, size_t mtry = 0, size_t p_categorical = 0, double kap = 16, double s = 4, bool verbose = false, bool parallel = true, bool set_random_seed = false, size_t random_seed = 0, bool sample_weights_flag = true, double nthread = 0,size_t ctType=1,size_t RepeatT=5,size_t StepSize=10, bool MHB = true, bool NWB = false,bool FITB=true)
{
    ...
    // define model
    NormalModel *model = new NormalModel(kap, s, tau, alpha, beta,num_trees);
    model->setNoSplitPenality(no_split_penality);

    // State settings
    std::vector<double> initial_theta(1, y_mean / (double)num_trees);
    std::unique_ptr<State> state(new NormalState(Xpointer, Xorder_std, N, p, num_trees, p_categorical, p_continuous, set_random_seed, random_seed, n_min, num_cutpoints, mtry, Xpointer, num_sweeps, sample_weights_flag, &y_std, 1.0, max_depth, y_mean, burnin, model->dim_residual, nthread, parallel,ctType,RepeatT,StepSize,MHB,NWB,FITB)); //last input is nthread, need update
    ...
    model->predict_std(ctType,Xtestpointer, N_test, p, num_trees, num_sweeps, yhats_test_xinfo, *trees2);
